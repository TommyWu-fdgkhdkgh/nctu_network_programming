...
class clientSession : public std::enable_shared_from_this<clientSession> {
  ...
  public:
    clientSession(char *name , char *port , char *filename ,int client_index):
	                                  s(io_context),
	                                  resolver(io_context),
					  filename(filename),
...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    for(int i=0;i<15;i+=3){
      if(parameters[i][0]!='\x00'){
        std::make_shared<clientSession>(parameters[i],parameters[i+1],parameters[i+2],i/3)->start();
      }
